...
|																                                  	|
|              modification by J.J 2025					          	|
|																                                  	|
_____________________________________________________________________

an ESP32 is used instead of an classic arduino Atmel AVR.
...
#define CORE_WIFI      1   // Core 1 : WiFi, MQTT, OTA
...
const long  gmtOffset_sec = 3600;      //  France H+1
const int   daylightOffset_sec = 0; // Summer Hour 0 if not used
...
{ // Begin setup
 ...
 // Mutex
  ...
  if (wifi_wait <20)
    ...
    Serial.print ("msec before midnight=") ;
...
xTaskCreatePinnedToCore(
    TaskUI, "TaskUI",
    20000,
    NULL, 2,          // priorité haute : temps réel
    NULL, CORE_REALTIME);

xTaskCreatePinnedToCore(
    Taskwifi, "wifi", 20000,
    NULL, 1,          // priorité basse : réseau
    NULL, CORE_WIFI);
...
bool Connect_MQTT()
{
  ...
  {
       client.disconnect();
    vTaskDelay(pdMS_TO_TICKS(2000)); // wait 2 secondes before connecting again.
