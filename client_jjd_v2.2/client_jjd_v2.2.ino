...
unsigned long unballasting_timeout = 600000; // timeout to avoid relay command too often 600 secondes 10 minutes
...
int Treshold_relay1 = 350000;          // Threshold to stop relay 350W
  ...
  if (long (millis() - unballasting_time > unballasting_timeout))
   {
     ...
     if (dim < unballasting_dim_min)  // DIM is minimum => power in SSR is maximum
      ...
      // 
      //
      if (rPower > Treshold_relay1) {         //if power is > treshold_relay1 (350W) stop relay 1
        //
