...
Test to verify scr; based on previous software without modification except 
scr_pin is set on/off
limite led (red) is set ON/off
SCRLED ON/off
relay 1 on/off
relay 2 ON/off
...
unsigned long timeonoff_now;
unsigned long timeonoff_limit = 1000 ; // time 1sec
volatile bool timeonoff = false;
...
timeonoff_now= millis(); // set up timer 
...
void loop()
 ...
 if (long (millis() - timeonoff_now > timeonoff_limit))
{
  if (timeonoff == false)
  {
